...
function searchCities(inputChar, database) {
    ...
    return database.filter(place => {
       let regex = new RegExp(inputChar, 'gi');
...
let inputForm = document.getElementById('search-input');
let suggestionsBox = document.getElementById('suggestions-box');

function displayCities() {
    let input = this.value;
    let output = searchCities(input,citiesDatabase);//return filtered array
    let addList =  output.map(function(place) {
        let regex = new RegExp(input, 'gi');
        const cityName = place.city.replace(regex, `<span class="hl">${input}</span>`);//highlight the matched char
